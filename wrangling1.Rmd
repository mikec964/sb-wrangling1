...
category <- c("p" = "Smartphone",
                        ...
                        "x" = "Laptop",
                        "q" = "Tablet")
products.norm <- add_column(products.norm, category[products.norm$p.code], 
                            .after="code")
...
```{r}
full.address <- paste(products.norm$address,
                       products.norm$city,
...
```{r}
cn <- sapply(real.names, function(x){ paste0("company_", x) })
products.norm <- mutate(products.norm, company_philips = products.norm$company == "philips")
products.norm <- mutate(products.norm, company_akzo = products.norm$company == "akzo")
products.norm <- mutate(products.norm, `company_van houten` = products.norm$company == "van houten")
products.norm <- mutate(products.norm, company_unilever = products.norm$company == "unilever")
```
